...
name2entrez <- getBM(attributes = c("external_gene_name", "entrezgene"), filters = "external_gene_name", values = circlesExpressed$host_gene, mart = mart)

circlesExpressed$external_gene_name<-circlesExpressed$host_gene
myCircles<-merge(circlesExpressed,name2entrez,by="external_gene_name",all.x=T)
...
save(myCircles,file="allMyCircles.Rdata")
#since this will take a while, only design primers for the ones I will actually use
allMyCircles<-myCircles
myCircles<-allMyCircles[which(allMyCircles$external_gene_name %in% c("Fbxw7","Ate1","Arhgap10")),]
myCircles<-myCircles[c(1,13,15),] # multiple circles per host, so select the correct one
...
primer3output<-readLines("150528_primer3output.txt")
...
myCircles_noNA<-myCircles_noNA#[which(myCircles$entrezgene!='NA'),]
...
file<-"150528_primer3input_seq_primers.txt"
for(row in 1:nrow(myCircles)){
  if(!is.na(myCircles[row,]$entrezgene)){
...
primer3output<-readLines("150528_primer3output_seq_primers.txt")
# subset it with a sequence, because the number of output lines is known
# note: working with a version of the myCircles dataframe that leaves out the entries without data, to make it match later
sel_left<-c(0:(nrow(myCircles_noNA)-1))*123 +16 #PRIMER_LEFT_0_SEQUENCE
sel_right<-c(0:(nrow(myCircles_noNA)-1))*123 +17 #PRIMER_RIGHT_0_SEQUENCE
